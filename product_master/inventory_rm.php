...
//echo $max_row."<br>".$max_col; # 313 and G
	...
	for ($i=2; $i <= $max_row ; $i++) {  
		$rowData = $sheet->rangeToArray('A' . $i . ':' . $max_col . $i);
			//print_r($rowData);
		$sql .= "('NULL','".$rowData[0][0]."', '".$rowData[0][1]."', '".$rowData[0][2]."', '".$rowData[0][3]."', 'R', '$fk_company'),";
	}
	$sql = rtrim($sql, ",");
	//echo $sql;
	$res = mysqli_query($db, $sql) or die("Something Went Wrong. The file was not uploaded. Please contact admin.");
	if ($res) {
		echo "<script>alert(\"Upload Success!\"); window.location.assign(\"inventory_rm.php\")</script>";
	}
}
...
<div class="container-fluid">
	<div class="row">
		<div class="col-sm-5 col-sm-offset-3">
			<fieldset>
				<legend>UPLOAD YOUR <span style="color: blue;">RAW MATERIALS</span> EXCEL FILE</legend>
				...
				<form method="POST" enctype="multipart/form-data">
					...
					<div class="form-group">
						<p><input type="submit" name="upld_sub1" class="btn btn-primary"><a href="../sampleSheet/SAMPLE_SHEET.xlsx" style="padding-left: 10px;">Click Here</a> to download sample file.</p>
					</div>
					<div class="form-group">
						<button type="button" class="btn btn-danger" onclick="ALERTME();">Delete all My Raw Materials</button>
					</div>
				</form>
			</fieldset>
		</div>
	...
	<div class="row" style="padding-top: 15px;">
		<div class="col-sm-12">
			<fieldset>
				<legend>YOUR RAW MATERIALS INV. RECORDS</legend>
				...
				<table class="table table-bordered">
					...
					<tbody>
						<?php $arik = mysqli_query($db, "SELECT * FROM inventory_card WHERE fk_company='$fk_company' and item_type='R'");
						$sl = 0;
...
function ALERTME() {
	...
	if (!person || person != '<?php echo($world[2]); ?>') {
		...
		window.location.assign("del_record.php?rm_id=R");
	}
}
</script>
</body>
